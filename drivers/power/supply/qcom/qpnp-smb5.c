...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	if (chg->dcin_uusb_over_gpio_en) {
		...
		if (!gpio_is_valid(chg->micro_usb_gpio)) {
			pr_debug(" micro_usb_gpio not specified\n");
		} else {
			rc = devm_gpio_request(chg->dev, chg->micro_usb_gpio,
						"micro_usb");
			if (rc)
				pr_debug("request micro_usb_gpio failed, rc=%d\n",
						rc);

			rc = gpio_direction_input(chg->micro_usb_gpio);
			if (rc)
				pr_debug("Unable to set dir for micro_usb_gpio\n");
		...
		if (!gpio_is_valid(chg->dc_9v_gpio)) {
			pr_debug("dc_9v_gpio not specified\n");
		} else {
			rc = devm_gpio_request(chg->dev, chg->dc_9v_gpio,
						"dc_9v");
			if (rc)
				pr_debug("Request dc_9v gpio failed, rc=%d\n",
					rc);

			rc = gpio_direction_input(chg->dc_9v_gpio);
			if (rc)
				pr_debug("unable to set dir for dc_9v gpio\n");
		...
		if (!gpio_is_valid(chg->usb_switch_gpio)) {
			pr_debug("usb_switch_gpio not specified\n");
		} else {
			rc = devm_gpio_request(chg->dev, chg->usb_switch_gpio,
						"usb_switch");
			if (rc)
				pr_debug("Request usb_switch gpio failed, rc=%d\n",
					rc);

			rc = gpio_direction_output(chg->usb_switch_gpio, 1);
			if (rc)
				pr_debug("Unable to set dir for usb_switch gpio\n");
		}
		...
		if (!gpio_is_valid(chg->usb_hub_33v_en_gpio)) {
			pr_debug("usb_hub_33v_en_gpio not specified\n");
		} else {
			rc = devm_gpio_request(chg->dev,
						chg->usb_hub_33v_en_gpio,
						"usb_hub_33v_en");
			if (rc)
				pr_debug("Request usb_hub_33v_en gpio failed, rc=%d\n",
					 rc);

			rc = gpio_direction_output(chg->usb_hub_33v_en_gpio, 1);
			if (rc)
				pr_debug("Unable to set dir for usb_hub_33v_en gpio\n");
		}
	}
			...
			enum dual_role_property prop, unsigned int *val)
{
	...
	case DUAL_ROLE_PROP_DR:
		*val = dr;
		break;
	default:
		pr_debug("dual role class get property %d not supported\n", prop);
		return -EINVAL;
	}
	...
	case DUAL_ROLE_PROP_MODE:
		...
		case DUAL_ROLE_PROP_MODE_UFP:
			if (chg->typec_mode >=
				...
				if (rc < 0) {
					...
					pr_debug("Failed to force UFP mode, rc=%d\n",
						rc);
				}
			}
			break;
		case DUAL_ROLE_PROP_MODE_DFP:
			if (chg->typec_mode < POWER_SUPPLY_TYPEC_SOURCE_DEFAULT
				...
				if (rc < 0) {
					chg->pr_swap_in_progress = false;
					pr_debug("Failed to force DFP mode, rc=%d\n",
						rc);
				}
			}
			break;
		...
		break;
	default:
		pr_debug("dual role class set property %d not supported\n", prop);
		rc = -EINVAL;
	}
...
static int smb5_init_dual_role_class(struct smb5 *chip)
{
	...
	if (IS_ERR(chg->dual_role)) {
		pr_debug("Couldn't register dual role class\n");
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (pval.intval && !chg->aicl_disable) {
		...
		pr_debug("battery present = %d AICL on\n", pval.intval);
	} else {
		val = 0;
		pr_debug("battery present = %d AICL off\n", pval.intval);
	}
...
static int lct_register_powermanger(struct smb_charger *chg)
{
    ...
    if (ret)
        pr_debug("[FB]Unable to register fb_notifier: %d", ret);
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		pr_debug("Couldn't initialize dual role class, rc=%d\n",
			rc);
		goto cleanup;
	}
